package vontus.magicbottle.config;

import org.bukkit.enchantments.Enchantment;
import org.bukkit.inventory.ItemStack;

import java.text.ParseException;

public class EnchantParser {
	private static final Enchantment POWER = Enchantment.POWER;
	private static final Enchantment FLAME = Enchantment.FLAME;
	private static final Enchantment INFINITY = Enchantment.INFINITY;
	private static final Enchantment PUNCH = Enchantment.PUNCH;
	private static final Enchantment CURSE_OF_BINDING = Enchantment.BINDING_CURSE;
	private static final Enchantment SHARPNESS = Enchantment.SHARPNESS;
	private static final Enchantment BANE_OF_THE_ARTHROPODS = Enchantment.BANE_OF_ARTHROPODS;
	private static final Enchantment SMITE = Enchantment.SMITE;
	private static final Enchantment DEPTH_STRIDER = Enchantment.DEPTH_STRIDER;
	private static final Enchantment EFFICIENCY = Enchantment.EFFICIENCY;
	private static final Enchantment UNBREAKING = Enchantment.UNBREAKING;
	private static final Enchantment FIRE_ASPECT = Enchantment.FIRE_ASPECT;
	private static final Enchantment FROST_WALKER = Enchantment.FROST_WALKER;
	private static final Enchantment KNOCKBACK = Enchantment.KNOCKBACK;
	private static final Enchantment FORTUNE = Enchantment.FORTUNE;
	private static final Enchantment LOOTING = Enchantment.LOOTING;
	private static final Enchantment LUCK_OF_THE_SEA = Enchantment.LUCK_OF_THE_SEA;
	private static final Enchantment LURE = Enchantment.LURE;
	private static final Enchantment MENDING = Enchantment.MENDING;
	private static final Enchantment WATER_BREATHING = Enchantment.RESPIRATION;
	private static final Enchantment PROTECTION = Enchantment.PROTECTION;
	private static final Enchantment BLAST_PROTECTION = Enchantment.BLAST_PROTECTION;
	private static final Enchantment FEATHER_FALLING = Enchantment.FEATHER_FALLING;
	private static final Enchantment FIRE_PROTECTION = Enchantment.FIRE_PROTECTION;
	private static final Enchantment PROJECTILE_PROTECTION = Enchantment.PROJECTILE_PROTECTION;
	private static final Enchantment SILK_TOUCH = Enchantment.SILK_TOUCH;
	private static final Enchantment SWEEPING_EDGE = Enchantment.SWEEPING_EDGE;
	private static final Enchantment THORNS = Enchantment.THORNS;
	private static final Enchantment CURSE_OF_VANISHING = Enchantment.VANISHING_CURSE;
	private static final Enchantment AQUA_AFFINITY = Enchantment.AQUA_AFFINITY;

	private static final String ANY_ENCHANTMENT = "ANY";

	private final Enchantment ench;
	private final boolean anyEnchant;

	private EnchantParser(Enchantment e) {
		ench = e;
		anyEnchant = false;
	}

	private EnchantParser(boolean anyEnchant) {
		ench = null;
		this.anyEnchant = anyEnchant;
	}

	public boolean canRepair(ItemStack item) {
		return anyEnchant || (ench != null && item.containsEnchantment(ench));
	}

	public static EnchantParser parseForBukkit(String enchantString) throws ParseException {
		EnchantParser ep;
		switch (enchantString) {
			case ANY_ENCHANTMENT:
				ep = new EnchantParser(true);
				break;
			case "POWER":
				ep = new EnchantParser(EnchantParser.POWER);
				break;
			case "FLAME":
				ep = new EnchantParser( EnchantParser.FLAME);
				break;
			case "INFINITY":
				ep = new EnchantParser( EnchantParser.INFINITY);
				break;
			case "PUNCH":
				ep = new EnchantParser( EnchantParser.PUNCH);
				break;
			case "CURSE_OF_BINDING":
				ep = new EnchantParser( EnchantParser.CURSE_OF_BINDING);
				break;
			case "SHARPNESS":
				ep = new EnchantParser( EnchantParser.SHARPNESS);
				break;
			case "BANE_OF_THE_ARTHROPODS":
				ep = new EnchantParser( EnchantParser.BANE_OF_THE_ARTHROPODS);
				break;
			case "SMITE":
				ep = new EnchantParser( EnchantParser.SMITE);
				break;
			case "DEPTH_STRIDER":
				ep = new EnchantParser( EnchantParser.DEPTH_STRIDER);
				break;
			case "EFFICIENCY":
				ep = new EnchantParser( EnchantParser.EFFICIENCY);
				break;
			case "UNBREAKING":
				ep = new EnchantParser( EnchantParser.UNBREAKING);
				break;
			case "FIRE_ASPECT":
				ep = new EnchantParser( EnchantParser.FIRE_ASPECT);
				break;
			case "FROST_WALKER":
				ep = new EnchantParser( EnchantParser.FROST_WALKER);
				break;
			case "KNOCKBACK":
				ep = new EnchantParser( EnchantParser.KNOCKBACK);
				break;
			case "FORTUNE":
				ep = new EnchantParser( EnchantParser.FORTUNE);
				break;
			case "LOOTING":
				ep = new EnchantParser( EnchantParser.LOOTING);
				break;
			case "LUCK_OF_THE_SEA":
				ep = new EnchantParser( EnchantParser.LUCK_OF_THE_SEA);
				break;
			case "LURE":
				ep = new EnchantParser( EnchantParser.LURE);
				break;
			case "MENDING":
				ep = new EnchantParser( EnchantParser.MENDING);
				break;
			case "WATER_BREATHING":
				ep = new EnchantParser( EnchantParser.WATER_BREATHING);
				break;
			case "PROTECTION":
				ep = new EnchantParser( EnchantParser.PROTECTION);
				break;
			case "BLAST_PROTECTION":
				ep = new EnchantParser( EnchantParser.BLAST_PROTECTION);
				break;
			case "FEATHER_FALLING":
				ep = new EnchantParser( EnchantParser.FEATHER_FALLING);
				break;
			case "FIRE_PROTECTION":
				ep = new EnchantParser( EnchantParser.FIRE_PROTECTION);
				break;
			case "PROJECTILE_PROTECTION":
				ep = new EnchantParser( EnchantParser.PROJECTILE_PROTECTION);
				break;
			case "SILK_TOUCH":
				ep = new EnchantParser( EnchantParser.SILK_TOUCH);
				break;
			case "SWEEPING_EDGE":
				ep = new EnchantParser( EnchantParser.SWEEPING_EDGE);
				break;
			case "THORNS":
				ep = new EnchantParser( EnchantParser.THORNS);
				break;
			case "CURSE_OF_VANISHING":
				ep = new EnchantParser( EnchantParser.CURSE_OF_VANISHING);
				break;
			case "AQUA_AFFINITY":
				ep = new EnchantParser(EnchantParser.AQUA_AFFINITY);
				break;
			default:
				throw new ParseException("The enchantment '" + enchantString + "' is not supported", 0);
		}
		return ep;
	}
}
